...
import validationEmail from "../helpers/email/validation.email.helpers.js";
...
export const registerController = async (req, res, next) => {
    try {
        ...
        validador
            .isString("name")
            .min_max_length("name", 4, 20)
            .isEmail("email")
            .isString("password")
            .min_max_length("password", 8, 30);
        ...
        await validationEmail(email, name);
...
export const verifyEmailController = async (req, res, next) => {
    try {
        ...
        if (!token_validation) {
            return next(
                new AppError('No se encontro el "token_validation"', 404)
            );
        }
        ...
        return res
            ...
            .json(new ApiResponse(200, "Se verifico el usuario con exito"));
    } catch (error) {
        next(error);
    }
};
...
export const loginController = async (req, res, next) => {
    try {
        ...
        return res.status(200).json(
            new ApiResponse(200, "Login exitoso", {
                name: user.name,
                accessToken: TOKEN,
            })
        );
...
export const forgotPasswordController = async (req, res, next) => {
    try {
        ...
        return res
            .status(200)
            .json(
                new ApiResponse(200, `Email de recuperacion enviado a ${email}`)
            );
    } catch (error) {
        next(error);
    }
...
export const updatePasswordController = async (req, res, next) => {
    try {
        ...
        validador.isString("password").min_max_length("password", 8, 30);
        const errores = validador.obtenerErrores();
        if (errores.length > 0) {
            return next(new AppError("Errores de validacion", 400, errores));
        }
        ...
        return res
            .status(200)
            .json(new ApiResponse(200, "Password cambiada con exito"));
    } catch (error) {
        next(error);
    }
};
